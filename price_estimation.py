...
data['yr_renovated'][data['yr_renovated'] == 0] = data[
    'yr_built'][data['yr_renovated'] == 0]
...
# droping these columns to get our optimal set of variables for
# exploratory analysis.
data = data.drop(['id', 'date', 'lat', 'long', 'yr_built',
                  'yr_renovated', 'data_year'], axis=1)
...
def zip_code_avg(X, y):
    ...
    map = temp.groupby(['zipcode', 'zipcodeavgprice']).size(
...
# plot the polynomial regression relation between the price and variables
# which we found correlated in our exploratory analysis.
...
# Custom function to plot the hyperparameters against the cross validated
# scores
...
ridge = Ridge(alpha=gm_cv.best_params_[
              'Ridge__alpha'], normalize=True, tol=0.1)
...
lasso = Lasso(alpha=gm_cv.best_params_[
              'Lasso__alpha'], normalize=True, tol=0.1)
clf = lasso
polyreg()
...
# testing our data using the K Nearest Neighbor Regresssor.
